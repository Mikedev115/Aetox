...
| `RunOnce(ctx, message)` | One chat turn, string in → reply out. **This is the desktop's whole surface.** Internally builds a `turn.Executor` per call via `wireStatusReporter`. |
