...
| `BootstrapProvider(BootstrapOptions)` ([bootstrap.go](bootstrap.go)) | The one call front ends make: provider name + model + base URL + **either** a key **or** a `Transport` → ready `Provider` (or error + warning). Both `cmd/aetox` and `internal/bootstrap` (for the desktop) route through it. |
| `Transport` ([factory.go](factory.go)) — `func(network http.RoundTripper) http.RoundTripper` | **The credential seam (§248 A3).** A provider built with one attaches no key and requires none: the host wraps the network in something that signs each request from its own store (`desktop/provider_forward.go`), and a remote engine's will carry the request to the screen instead. Sits under the retry layer, so a retry is re-signed. **This package reads no credential store** — a sign-in's `TokenSource`/`Headers`/`SignedInEndpoint` are fields the *caller* fills from `internal/oauth`, which is why the factory no longer imports it. `AuthScheme(provider, wireFormat)` tells a signer which header the wire format carries a key in. |
