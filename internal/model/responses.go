...
func convertMessagesToResponses(msgs []Message) (instructions string, input []responsesItem) {
	...
	for _, m := range msgs {
		...
		default:
			content := make([]responsesContent, 0, len(m.Images)+1)
...
// humanizeDuration renders a reset window at the granularity a person acts on:
// "4 days" changes the decision, "330064 seconds" does not.
