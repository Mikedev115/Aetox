...
// the weekly one running out while the five-hour one still looks fine.
...
func TestAnthropicQuotaPrefersUnifiedWindow(t *testing.T) {
	...
	if got[0].Window != "week" || got[0].RemainingPercent != 30 {
		t.Errorf("got %+v; want the unified week window at 30%%", got[0])
	}
}
