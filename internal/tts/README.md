...
| `Read(ctx, eng, text, opts)` ([reader.go](reader.go)) | The streaming layer **above** `Engine` — synthesizes the pieces into files, up to `opts.Parallel` at once, and sends each on a channel in order as it lands. A piece the voice fails climbs `opts.Fallbacks` (built once, on the first failure) instead of ending the read; a voice that failed one piece is not offered the next, and each piece says which voice made it and what type its file is. Cancelling the context stops it mid-synthesis and climbs nothing. Optional shared cache so a second read of the same text synthesizes nothing — a stand-in's piece is cached under the stand-in's own key. |
...
- **No policy in here.** Which voice matches the UI language is [desktop/voice.go](../../desktop/voice.go)'s judgement — that is where the locale lives. An engine speaks with the voice it is told, or its own default. The same rule is why `Read` does not decide how far ahead of the listener to synthesize, nor how many pieces to have in the engine at once: its channel is buffered by one, a parallel slot frees only when its piece is taken, and the look-ahead window, the width, and the list of voices to fall back to (`speechFallbacks`) all belong to [desktop/speak.go](../../desktop/speak.go), where the listener is.
...
- **Cloud rows are labeled cloud, and say what leaves.** The owner opened the catalog to cloud vendors on 2026-09-01 (§216): Edge and gTTS free with no key; OpenAI, Groq (PlayAI, English only), Gemini (multilingual incl. Thai, PCM wrapped to WAV in-engine) and ElevenLabs on the user's own key. Every cloud row's Label carries "คลาวด์" and its Install text says the text goes out. Microsoft Edge is the shipped default because it needs no key or install and includes Thai neural voices; the local Windows engine remains an explicit offline choice. The Gemini engine skips the base-URL override on purpose: the provider row may point at the OpenAI-compatible chat endpoint, and this is a native-API call.
- **Keys come from the store the models page fills** (`credentials.ProviderAPIKey`), with the provider's usual env vars as fallback; base URLs honor the same per-provider override, so the OpenAI row pointed at a LocalAI/Speaches box speaks through it. A pip-installed CLI vendor is also found in `%APPDATA%\Python\*\Scripts`, which pip fills without putting on PATH.
- **Edge is spoken natively** ([edge.go](edge.go), since 2026-09-06): the Read Aloud websocket protocol as edge-tts documents it, with no CLI and no Python — the CLI cost a one-second interpreter start per piece. Nothing to install; the constants Microsoft may change sit at the top of the file, and `AETOX_EDGE_LIVE=1 go test ./internal/tts -run TestEdgeLive -v` is the check when they do. gTTS still goes through `gtts-cli`.
