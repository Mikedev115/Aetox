...
func newSwitchApp(t *testing.T) *Engine {
	t.Helper()
	t.Setenv("AETOX_DATA_ROOT", t.TempDir())
	isolateUserDirs(t)
	a := seed(&Engine{ctx: nil, emit: func(string, ...any) {}, dbDir: t.TempDir()}, &conversation{id: newSessionID()})
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
...
// reach (Engine.resolveConfig, shippedDelegation), which is a different fact and
