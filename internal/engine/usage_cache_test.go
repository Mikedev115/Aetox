...
func TestOpeningAnOlderDatabaseAddsMissingColumns(t *testing.T) {
	...
	a := seed(&Engine{cfg: config.Config{ModelName: "new-model"}, dbDir: dir}, newConversation())
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
...
func TestAddedColumnsIsIdempotent(t *testing.T) {
	a := seed(&Engine{cfg: config.Config{ModelName: "m"}, dbDir: t.TempDir()}, newConversation())
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
...
func TestUnreportedCacheStaysDistinctFromZero(t *testing.T) {
	a := seed(&Engine{cfg: config.Config{ModelName: "ollama-model"}, dbDir: t.TempDir()}, newConversation())
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
...
func TestDailySeriesIsAggregatedPerDay(t *testing.T) {
	a := seed(&Engine{cfg: config.Config{ModelName: "m1"}, dbDir: t.TempDir()}, newConversation())
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
...
func TestDailySeriesCarriesTheCacheSplit(t *testing.T) {
	a := seed(&Engine{cfg: config.Config{ModelName: "api-model"}, dbDir: t.TempDir()}, newConversation())
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
