...
func (a *Engine) prClient() *gh.PRClient {
...
func (a *Engine) SuggestPRDetails(head, base string) (PRSuggestion, error) {
	root, ok := a.gitRoot()
	if !ok {
...
func (a *Engine) ReviewPullRequest(number int) (string, error) {
	if number <= 0 {
...
func (a *Engine) PullRequestFiles(number int) []gh.PRFile {
...
func (a *Engine) PullRequestChecks(sha string) []gh.CheckRun {
...
func (a *Engine) CreatePullRequest(title, head, base, body string, draft bool) PRCreated {
	var room PRRoom
	repo, ok := a.roomRepo(&room)
	if !ok {
...
func (a *Engine) roomRepo(room *PRRoom) (gh.Repo, bool) {
	if gh.Token() == "" {
