...
func readRuns(t *testing.T, a *Engine) []storedRun {
	t.Helper()
	db, err := a.database()
	if err != nil {
...
func newRunApp(t *testing.T) *Engine {
	t.Helper()
	a := seed(&Engine{cfg: config.Config{}, dbDir: t.TempDir()}, &conversation{id: "20260801-120000.000"})
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
