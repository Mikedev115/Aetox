...
func TestStudioHostServesByIdAndNothingElse(t *testing.T) {
	...
	// Mounted at the engine's own prefix; the screen's /aetox-shelf/ is a
	// proxy onto it (rpc.FileProxy), so studioHostPrefix here stands for
	// whatever prefix the handler was given.
	get := func(target string) *httptest.ResponseRecorder {
		rec := httptest.NewRecorder()
		a.shelfHandler(studioHostPrefix).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, target, nil))
		return rec
	}
	...
	// Anything outside the prefix is not this handler's to answer.
	if rec := get("/aetox-file/x.png"); rec.Code != http.StatusNotFound {
