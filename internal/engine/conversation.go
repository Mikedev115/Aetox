...
//
// Aetox was the opposite of that for its whole life. `Engine` held one agent, one
...
//
// Every field here was an `Engine` field until 2026-08-19. They moved together
// because they were always one thing — none of them describes the app, all of
// them describe a chat — and keeping half of them on `Engine` would leave exactly
...
type conversation struct {
	...
	//
	// The dials moved here on 2026-08-20. They were `Engine.cfg` alone, which was
	...
	//
	// `Engine.cfg` is still there and still means something, but something else:
	...
	// widened mid-turn waits (§185). Guarded by Engine.turnMu, not free-standing:
	...
	// Engine.turnMu with pendingCfg, and read in the same breath.
	stanceRebuild bool
	...
	// first result on its second queue. All three are guarded by Engine.turnMu
	...
	//
	// It was one field on Engine, which was true while one turn could exist. With
	...
	// back to. Guarded by Engine.snapshotMu with the store.
	lastSnapshot string
	...
	// editor, because Engine.WriteFile is the only door it has. So undo keeps its
	...
	// the chat being looked at. Guarded by Engine.snapshotMu.
	//
	...
	// made that claim false. Guarded by Engine.toolHistoryMu, which covers every
	...
	// whole turn is assembled. Guarded by Engine.toolHistoryMu, like the slice
	...
	// without either one seeing the other's. Guarded by Engine.askMu.
	askCh chan string
}
...
// shape was considered and rejected: it makes `Engine` the answer to "which chat
...
func (a *Engine) cur() *conversation {
	// Once, not a bare nil check. NewEngine builds the manager, so in the running
	// app this does nothing at all — but a zero Engine is how most of this
...
// different lock (Engine.snapshotMu) while it walks the result, and handing out
