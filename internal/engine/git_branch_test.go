...
func branchApp(root string) *Engine {
	return seed(&Engine{cfg: config.Config{SandboxRoot: root}, projectFocused: true}, newConversation())
}
...
func TestBranchesAreEmptyWhereThereIsNothingToChooseFrom(t *testing.T) {
	...
	unfocused := seed(&Engine{cfg: config.Config{SandboxRoot: root}}, newConversation())
...
func TestSwitchingRefusesWithoutAFocusedProject(t *testing.T) {
	a := seed(&Engine{cfg: config.Config{SandboxRoot: repoWithBranches(t)}}, newConversation())
