...
func (a *Engine) WorkspaceFolders() []WorkspaceFolder {
...
// without a window.
func (a *Engine) AddWorkspaceFolderAt(dir string) ([]WorkspaceFolder, error) {
	if !a.projectFocused {
...
func (a *Engine) recordWorkspaceFolder(dir string) error {
...
func (a *Engine) RemoveWorkspaceFolder(path string) ([]WorkspaceFolder, error) {
	root := strings.TrimSpace(a.cur().cfg.SandboxRoot)
...
// the panel.
func (a *Engine) workspaceRoots() []string {
	a.wsMu.Lock()
	defer a.wsMu.Unlock()
	return append([]string(nil), a.extraRoots...)
}

func (a *Engine) setWorkspaceRoots(roots []string) {
	a.wsMu.Lock()
	defer a.wsMu.Unlock()
	a.extraRoots = roots
}
...
func (a *Engine) askWorkspaceWiden(conv *conversation, target string) bool {
...
func (a *Engine) markWorkspaceDirty() {
	a.wsMu.Lock()
	defer a.wsMu.Unlock()
...
func (a *Engine) takeWorkspaceDirty() bool {
	a.wsMu.Lock()
	defer a.wsMu.Unlock()
...
func (a *Engine) storeWorkspaceFolder(root, dir string) error {
	db, err := a.database()
	if err != nil {
		return err
	}
...
func (a *Engine) storedWorkspaceFolders(root string) []string {
