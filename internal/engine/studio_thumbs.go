...
func (a *Engine) StudioThumbs(ids []string) map[string]string {
...
func (a *Engine) runThumbWorker(th assetlib.Thumber) {
	for {
		a.thumbs.mu.Lock()
...
// serveStudioThumb answers a poster request — rest is the path under the
// shelf's mount — or reports false for a path that is not one so the shelf
// host continues with its own lookup.
func serveStudioThumb(w http.ResponseWriter, r *http.Request, rest string) bool {
	if !strings.HasPrefix(rest, "thumb/") {
		return false
	}
	name := strings.TrimPrefix(rest, "thumb/")
