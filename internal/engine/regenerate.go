...
// asks it once.
func (a *Engine) RetryFailedTurn(text string) (TurnReply, error) {
...
// revert.
func (a *Engine) RegenerateReply(revertFiles bool) (RegenerateResult, error) {
...
func (a *Engine) ResendEdited(text string, revertFiles bool) (TurnReply, error) {
	if strings.TrimSpace(text) == "" {
...
func (a *Engine) SwitchVariant(index int) (RegenerateResult, error) {
...
func (a *Engine) lastQuestion() (string, bool) {
...
func (a *Engine) restoreContext(conv *conversation, messages []SessionMessage) {
	if a.cur().agent == nil {
		return
	}
...
func (a *Engine) storeVariants(conv *conversation, variants []SessionVariant, active int) {
...
func (a *Engine) storeParts(conv *conversation, parts []turn.TurnPart) {
	sessionID := conv.id
	db, err := a.database()
...
func (a *Engine) dropLastTurnRows() {
	db, err := a.database()
	if err != nil || a.cur().id == "" {
		return
	}
