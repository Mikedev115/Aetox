...
func listPath(t *testing.T, a *Engine, path string) (skill.Output, error) {
	t.Helper()
...
func focusedApp(t *testing.T, root string) *Engine {
	t.Helper()
	a := seed(&Engine{dbDir: t.TempDir(), projectFocused: true}, newConversation())
...
func TestWorkspaceFoldersAreStoredPerProject(t *testing.T) {
	a := seed(&Engine{dbDir: t.TempDir()}, newConversation())
	closeDBOnCleanup(t, a)
