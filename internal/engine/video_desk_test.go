...
// deskApp is an Engine whose desk events are collected instead of emitted.
func deskApp(t *testing.T) (*Engine, *conversation, *[]MediaOrigin) {
	t.Helper()
	root := t.TempDir()
...
func TestCuttingRoomToolOpensTheRoom(t *testing.T) {
	var got []string
	a := seed(&Engine{emit: func(event string, data ...any) {
		if len(data) > 0 {
...
func TestDeskMediaMatchesThePanes(t *testing.T) {
	src, err := os.ReadFile(filepath.Join("..", "..", "desktop", "frontend", "src", "lib", "stores", "workbench.svelte.ts"))
	if err != nil {
		t.Fatal(err)
	}
