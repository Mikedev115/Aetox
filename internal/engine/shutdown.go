...
// run out.
func (a *Engine) beforeClose(_ context.Context) (prevent bool) {
...
func (a *Engine) finishTurnsForClose(grace time.Duration) bool {
...
// engineCtx is the engine's lifetime (Engine.lifeCtx): the parent of every turn
// and of every piece of engine work that runs outside one. Never nil, never
// the window's context.
func (a *Engine) engineCtx() context.Context {
	a.lifeOnce.Do(func() {
...
func (a *Engine) stopEverything() int {
	a.turnMu.Lock()
...
func (a *Engine) allConversations() []*conversation {
	a.cur() // builds a.convs once, for the zero Engine the tests construct
	return a.convs.all()
}
...
func (a *Engine) closeReason(err error) error {
