...
func (a *Engine) SessionSources(sessionID string) []Source {
	out := []Source{}
...
func (a *Engine) SessionSourceCount(sessionID string) int {
	return len(a.allSources(sessionID))
}

func (a *Engine) allSources(sessionID string) []Source {
	if strings.TrimSpace(sessionID) == "" {
		return nil
	}
	db, err := a.database()
