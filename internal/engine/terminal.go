...
func (a *Engine) TerminalShells() []ShellProfile {
	out := []ShellProfile{}
...
func (a *Engine) TerminalStart(shellPath string, cols, rows int) (string, error) {
	if cols <= 0 {
		cols = 80
	}
	if rows <= 0 {
...
func (a *Engine) pumpTerminalOutput(s *TerminalSession) {
	buf := make([]byte, 4096)
	for {
...
func (a *Engine) TerminalAttach(sessionID string) string {
	a.terminalsMu.Lock()
...
func (a *Engine) closeSession(id string) {
	a.terminalsMu.Lock()
	s, ok := a.terminals[id]
	if ok {
...
func (a *Engine) getSession(id string) (*TerminalSession, error) {
	a.terminalsMu.Lock()
	s, ok := a.terminals[id]
	a.terminalsMu.Unlock()
	if !ok {
...
func (a *Engine) TerminalWrite(sessionID, data string) error {
...
func (a *Engine) TerminalResize(sessionID string, cols, rows int) error {
	s, err := a.getSession(sessionID)
	if err != nil {
		return err
	}
...
func (a *Engine) TerminalClose(sessionID string) error {
...
func (a *Engine) shutdown(_ context.Context) {
