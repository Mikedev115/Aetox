...
// directly instead, which it could not be before Engine.emit existed:
...
func newTestTerminalSession(t *testing.T) (*Engine, string) {
	t.Helper()
	a := &Engine{terminals: map[string]*TerminalSession{}}
...
func TestCloseSessionIsIdempotent(t *testing.T) {
	a := &Engine{terminals: map[string]*TerminalSession{}, emit: func(string, ...any) {}}
	f := &fakePTY{}
	id := nextTerminalID()
