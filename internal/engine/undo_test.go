...
// undoApp is the Engine reduced to what undo touches: a real git project and a
// real store. Nothing is stubbed — the whole claim of this feature is about
// what happens to files on disk.
func undoApp(t *testing.T) *Engine {
	t.Helper()
	...
	return &Engine{ctx: context.Background(), snapshots: store}
}
...
func TestUndoWithoutASnapshotStoreExplainsItself(t *testing.T) {
	app := &Engine{ctx: context.Background()}
