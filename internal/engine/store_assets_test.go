...
const storeAssetsDir = "../../desktop/build/windows/msix/Assets"
...
func TestStoreIconsReuseTheHandDrawnFrameWhereThereIsOne(t *testing.T) {
	frames := icoFrames(t, filepath.Join("..", "..", "desktop", "build", "windows", "icon.ico"))
	checked := 0
	for _, size := range taskbarSizes {
...
func TestTheStorePackageHasAPriConfigToIndexWith(t *testing.T) {
	path := filepath.Join("..", "..", "desktop", "build", "windows", "msix", "priconfig.xml")
