...
func aetoxSkillBody(t *testing.T) string {
	t.Helper()
	path := filepath.Join("..", "skill", "skills", "aetox", "SKILL.md")
...
func TestTheAetoxSkillNamesTheRealDataRootPaths(t *testing.T) {
	...
	cases := []struct {
		...
		{"desks", mode.Dir},
		{"เอเจน", config.AgentsRoot},
		{"ซับเอเจน", subagent.Dir},
		{"โปรเจกต์", spacesRoot},
...
func TestTheAetoxSkillNamesTheRealDatabaseFile(t *testing.T) {
	root := t.TempDir()
	t.Setenv("AETOX_DATA_ROOT", root)
	a := &Engine{ctx: context.Background(), emit: func(string, ...any) {}}
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
...
func TestTheAetoxSkillNamesTheRealOutputLocation(t *testing.T) {
	...
	a := seed(&Engine{ctx: context.Background(), emit: func(string, ...any) {}}, &conversation{id: newSessionID()})
	sub := a.outputSubdir()
	if sub == "" {
...
func TestTheBundledSkillIsNotInTheToolBlock(t *testing.T) {
	...
	a := seed(&Engine{ctx: context.Background(), emit: func(string, ...any) {}, dbDir: t.TempDir()}, &conversation{id: newSessionID()})
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
