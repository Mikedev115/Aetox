...
//
// Since §248 the shelf is where the engine is: the handler is served on the
// engine's own listener at rpc.ShelfPath and the screen's /aetox-shelf/ is a
// proxy onto it (rpc.FileProxy), the same road the project's files take —
// so a shelf on a host over ssh is browsed and played exactly like one here.
...
// asset id alone decides which file answers. This is the address the
// webview uses — the screen's — and what the engine's thumbnail URLs carry;
// the engine's listener mounts the handler at its own prefix (ShelfHandler)
// and the screen's proxy rewrites one into the other.
const studioHostPrefix = "/aetox-shelf/"

// ShelfHandler serves the studio's shelf under prefix: one catalogued file
// per request, by id, and the posters. Mounted by rpc.Server beside the
// project's files.
func ShelfHandler(e *Engine, prefix string) http.Handler { return e.shelfHandler(prefix) }

func (a *Engine) shelfHandler(prefix string) http.Handler {
