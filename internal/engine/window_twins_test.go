package engine

// What the window's packs ask of the engine (window_twins.go). The browser
// takes the picture; where it goes in the project is decided here, which is
// why these tests moved in from the browser's own file with §248 B1.

import (
	"os"
	"path/filepath"
	"strings"
	"testing"

	"github.com/Mikedev115/Aetox/internal/config"
)

// A screenshot is a byproduct of the agent looking at something, not a file the
// user asked for by name — so it goes to output/<session> even with a project
// focused, where a.outputSubdir() would put a new file in the project root.
// Getting this wrong drops page-1.png into the root of somebody's repository.
func TestBrowserShotStaysOutOfTheProjectRoot(t *testing.T) {
	root := t.TempDir()
	a := seed(&Engine{cfg: config.Config{SandboxRoot: root}, projectFocused: true}, &conversation{id: "s1"})

	rel, err := a.SaveBrowserShot([]byte("\x89PNG fake"), false)
	if err != nil {
		t.Fatalf("SaveBrowserShot() = %v", err)
	}
	if !strings.HasPrefix(rel, "output/s1/") {
		t.Errorf("SaveBrowserShot() = %q, want it under output/s1 — output/<session> is the one folder ListArtifacts sweeps", rel)
	}
	if !strings.HasSuffix(rel, ".png") {
		t.Errorf("SaveBrowserShot() = %q, want a .png", rel)
	}
	if strings.Contains(rel, "..") {
		t.Errorf("SaveBrowserShot() = %q, which climbs out of the sandbox", rel)
	}

	abs := filepath.Join(root, filepath.FromSlash(rel))
	got, err := os.ReadFile(abs)
	if err != nil {
		t.Fatalf("the path it reported does not exist: %v", err)
	}
	if string(got) != "\x89PNG fake" {
		t.Errorf("the file holds %q, not the bytes handed in", got)
	}
	if entries, _ := os.ReadDir(root); len(entries) != 1 || entries[0].Name() != "output" {
		t.Errorf("the project root gained something other than output/: %v", entries)
	}
}

// A screenshot is not a deliverable, and the gallery has one way of knowing
// that: the folder it is in. If this ever writes flat into output/<session>
// again, the shots go back to sitting level with the document somebody asked
// for — 46 of 244 cards on the owner's machine, the day this was written.
func TestBrowserShotGoesInTheWorkFolderSoTheGalleryCanStackIt(t *testing.T) {
	root := t.TempDir()
	a := seed(&Engine{cfg: config.Config{SandboxRoot: root}, projectFocused: true}, &conversation{id: "s1"})

	rel, err := a.SaveBrowserShot([]byte("PNG fake"), false)
	if err != nil {
		t.Fatalf("SaveBrowserShot() = %v", err)
	}
	if want := "output/s1/" + workSubdir + "/"; !strings.HasPrefix(rel, want) {
		t.Errorf("SaveBrowserShot() = %q, want it under %q", rel, want)
	}

	// And the gallery has to read that folder back as the deck key. Going
	// through ListArtifacts rather than asserting on folderUnder directly is
	// the point: the two halves are only useful if they agree.
	found := false
	for _, art := range sweepSession(filepath.Join(root, "output", "s1"), "s1", root) {
		if filepath.Base(art.Path) != filepath.Base(rel) {
			continue
		}
		found = true
		if art.Folder != workSubdir {
			t.Errorf("the shot reports Folder %q, want %q — the gallery groups on this", art.Folder, workSubdir)
		}
	}
	if !found {
		t.Error("the shot never came back from the sweep at all")
	}
}

// Two shots in one turn are two files. They were one for as long as the name
// was fixed, which reads as the second capture having failed silently.
func TestTwoShotsAreTwoFiles(t *testing.T) {
	a := seed(&Engine{cfg: config.Config{SandboxRoot: t.TempDir()}}, &conversation{id: "s1"})

	first, err := a.SaveBrowserShot([]byte("one"), false)
	if err != nil {
		t.Fatalf("SaveBrowserShot() = %v", err)
	}
	second, err := a.SaveBrowserShot([]byte("two"), false)
	if err != nil {
		t.Fatalf("SaveBrowserShot() = %v", err)
	}
	if first == second {
		t.Errorf("both shots went to %q — the first is gone", first)
	}
}

// A chat that has not been saved yet still has to be able to photograph a page.
func TestBrowserShotWorksBeforeASessionHasAnID(t *testing.T) {
	a := seed(&Engine{cfg: config.Config{SandboxRoot: t.TempDir()}}, newConversation())

	if _, err := a.SaveBrowserShot([]byte("png"), false); err != nil {
		t.Errorf("SaveBrowserShot() with no session id = %v", err)
	}
}

// No working folder is the one case where there is nowhere to put it, and the
// refusal has to be a sentence rather than a file written who-knows-where.
func TestBrowserShotRefusesWithNoWorkingFolder(t *testing.T) {
	a := seed(&Engine{}, &conversation{id: "s1"})

	if rel, err := a.SaveBrowserShot([]byte("png"), false); err == nil {
		t.Errorf("SaveBrowserShot() with no sandbox root wrote to %q", rel)
	}
}

// The file on disk is named for what it is. A gallery where the page and the
// page-with-numbers-on-it are both page-4.png is a gallery in which nobody can
// answer "what was the agent looking at when it clicked that".
func TestMarkedShotIsNamedForWhatItIs(t *testing.T) {
	a := seed(&Engine{cfg: config.Config{SandboxRoot: t.TempDir()}}, &conversation{id: "s1"})

	plain, err := a.SaveBrowserShot([]byte("PNG"), false)
	if err != nil {
		t.Fatal(err)
	}
	marked, err := a.SaveBrowserShot([]byte("PNG marked"), true)
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(plain, "-marks") {
		t.Errorf("plain shot = %q", plain)
	}
	if !strings.HasSuffix(marked, "-marks.png") {
		t.Errorf("marked shot = %q, want it named for the marks", marked)
	}
}

// The page-marks switch is the engine's preference, read by the browser
// through the twin: on by default, and off when the user turned it off.
func TestPageMarksSwitchIsTheEnginesToAnswer(t *testing.T) {
	if !(&Engine{}).PageMarksOn() {
		t.Error("the layer ships on and reports off")
	}
	off := &Engine{cfg: config.Config{BusyPageMarksOff: true}}
	if off.PageMarksOn() {
		t.Error("the layer was switched off and reports on")
	}
}
