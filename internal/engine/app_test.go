...
func TestGetContextBreakdownReportsAnUnknownWindowAsUnknown(t *testing.T) {
	...
	a := seed(&Engine{}, &conversation{agent: agent})
...
func TestGetContextBreakdownPinsFixedSlicesAndReportsCache(t *testing.T) {
	...
	a := seed(&Engine{dbDir: t.TempDir()}, &conversation{agent: agent, id: "s1"})
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
...
func TestUnfocusedRootAndOutputSubdirComposeToTheSessionFolder(t *testing.T) {
	...
	a := seed(&Engine{}, &conversation{id: "20260726-063257.366"})
...
func TestUnfocusedEngineRoamsFocusedEngineStaysWalled(t *testing.T) {
	...
	listOutside := func(a *Engine) (skill.Output, error) {
		t.Helper()
	...
	a := &Engine{} // zero value: projectFocused=false, the unfocused startup state
	closeStoreAtEnd(t, a)
...
func TestGitChangedFilesEmptyWhenUnfocused(t *testing.T) {
	a := seed(&Engine{cfg: config.Config{SandboxRoot: t.TempDir()}}, newConversation()) // projectFocused: false
...
func TestEnabledProvidersDefaultsToActiveProviderFromCfg(t *testing.T) {
	...
	a := seed(&Engine{cfg: config.Config{ModelProvider: "deepseek"}}, newConversation())
...
func TestReloadLoadsTheSavedModelOnFirstBootstrap(t *testing.T) {
	...
	a := &Engine{} // nothing bootstrapped yet — this is Engine.startup
	closeStoreAtEnd(t, a)
...
func TestInterjectHandsTheTextToTheRunningAgent(t *testing.T) {
	...
	if err := (seed(&Engine{}, &conversation{modelStatus: "no key"})).Interject("hello"); err == nil {
...
func TestNotAllowedToLookIsNotTheSameAsGone(t *testing.T) {
	...
	if got := (&Engine{}).FileStillThere("anything.txt"); got != FileUnknown {
...
func TestGetModelInfoNeverReportsAnEmptyApproval(t *testing.T) {
	a := &Engine{} // the pre-startup state: no config built yet
