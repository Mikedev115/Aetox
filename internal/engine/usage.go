...
func (a *Engine) emitUsageRound(conv *conversation, u model.Usage) {
	if conv == nil {
		return
	}
...
// that will not open.
func (a *Engine) SessionSpend(id string) SessionSpend {
...
func (a *Engine) priceRound(conv *conversation, u model.Usage) (float64, bool) {
...
func (a *Engine) UsageStats() (UsageStats, error) {
	var out UsageStats
	db, err := a.database()
	if err != nil {
		return out, err
...
func (a *Engine) priceUsage(out *UsageStats) {
	catalog := a.modelCatalog()
	if catalog == nil {
		return
	}
...
func (a *Engine) PriceModels(providerName string, models []string) []ModelListing {
...
func (a *Engine) ModelPriceSource() PriceSource {
	catalog := a.modelCatalog()
...
// schedule.
func (a *Engine) modelCatalog() *model.ModelCatalog {
	a.pricesMu.Lock()
...
func (a *Engine) RefreshModelFacts() {
	root, err := config.DataRoot()
	if err != nil {
		return
	}
