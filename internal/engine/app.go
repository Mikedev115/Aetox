...
type Engine struct {
	...
	// to stop it.
	//
	// Built lazily through goalRunSet() rather than in NewEngine, because most of
	// this package's tests construct a zero Engine and every one of them would
	...
	// die with a window that merely reconnects. Made lazily so an Engine built by
	...
	// same file, and two records could not see each other. Built once in NewEngine
	...
	// a second server over the first (engine_server.go). On the Engine because the
...
func (a *Engine) recordToolAction(conv *conversation, ev turn.ToolEvent) {
...
func (a *Engine) previewAnswer(conv *conversation, chunk string) { a.emitChatChunk(conv, chunk, false) }

// discardAnswerPreview erases it, for every round whose text turns out not to be
// the answer.
func (a *Engine) discardAnswerPreview(conv *conversation) { a.emitChatChunk(conv, "", true) }
...
func (a *Engine) AppVersion() string { return version.Current }
...
func (a *Engine) AppCredit() string { return version.Credit }
...
func (a *Engine) GitChangedFiles() []ChangedFile {
	out := []ChangedFile{}
...
func (a *Engine) ProjectTree() []TreeNode {
...
func (a *Engine) RelativizePath(absPath string) (string, error) {
	root := strings.TrimSpace(a.cur().cfg.SandboxRoot)
	if root == "" {
...
func (a *Engine) FileStillThere(relPath string) string {
	root := strings.TrimSpace(a.cur().cfg.SandboxRoot)
	if root == "" {
		return FileUnknown
	}
...
func (a *Engine) ReadWorkbook(relPath string) (*ooxml.WorkbookPreview, error) {
	root := strings.TrimSpace(a.cur().cfg.SandboxRoot)
	if root == "" {
...
func (a *Engine) ReadFile(relPath string) (string, error) {
	root := strings.TrimSpace(a.cur().cfg.SandboxRoot)
	if root == "" {
		return "", fmt.Errorf("no project open")
	}
...
func (a *Engine) WriteFile(relPath, content string) error {
	root := strings.TrimSpace(a.cur().cfg.SandboxRoot)
	if root == "" {
		return fmt.Errorf("no project open")
	}
...
//
// Lazily rather than in NewEngine because most of this package's tests construct a
// zero Engine, and a nil record there would quietly disable the guard in exactly
// the tests written to check it.
func (a *Engine) fileState() *skill.FileState {
	a.filesOnce.Do(func() {
		if a.files == nil {
...
func (a *Engine) SaveChatImage(sourcePath string) (string, error) {
...
func (a *Engine) SaveChatFile(sourcePath string) (string, error) {
...
func (a *Engine) SaveChatImageData(dataURL string) (string, error) {
...
func (a *Engine) chatAttachmentDest(ext string) (destPath, root string, err error) {
	root = strings.TrimSpace(a.cur().cfg.SandboxRoot)
	if root == "" {
		return "", "", fmt.Errorf("no project open")
	}
...
func (a *Engine) saveChatAttachment(sourcePath string, maxBytes int64) (string, error) {
...
func (a *Engine) ReadImageDataURL(relPath string) (string, error) {
	root := strings.TrimSpace(a.cur().cfg.SandboxRoot)
	if root == "" {
		return "", fmt.Errorf("no project open")
	}
...
func (a *Engine) visionAttachments(text string) (string, []model.Image) {
...
func (a *Engine) documentAttachments(text string) (string, []model.Document) {
...
func (a *Engine) captureSnapshot(conv *conversation, label string) {
	a.snapshotMu.Lock()
	store := a.snapshots
	a.snapshotMu.Unlock()
	if store == nil {
...
func (a *Engine) recordPoint(conv *conversation, id, label string) {
	a.snapshotMu.Lock()
...
func (a *Engine) noteUserSave(path string) {
...
func (a *Engine) planUndo(conv *conversation, store *snapshot.Store, id string, after int) (undoPlan, error) {
...
func (a *Engine) UndoLastTurn() (UndoResult, error) {
...
func (a *Engine) RewindTo(id string) (UndoResult, error) {
...
func (a *Engine) rewind(conv *conversation, store *snapshot.Store, id string, index int, nothingChanged string) (UndoResult, error) {
	if id == "" {
		return UndoResult{Reason: "nothing to undo yet"}, nil
	}
...
func (a *Engine) pointIndex(conv *conversation, id string) int {
	a.snapshotMu.Lock()
	defer a.snapshotMu.Unlock()
	for i, p := range conv.restorePoints {
		if p.ID == id {
...
func (a *Engine) RestorePoints() []RestorePoint {
	conv := a.cur()
	a.snapshotMu.Lock()
	defer a.snapshotMu.Unlock()
...
func (a *Engine) PendingRestore(id string) []string {
	id = strings.TrimSpace(id)
	conv := a.cur()
	a.snapshotMu.Lock()
	store := a.snapshots
...
func (a *Engine) PendingUndo() []string {
	conv := a.cur()
	a.snapshotMu.Lock()
	store, id := a.snapshots, conv.lastSnapshot
	a.snapshotMu.Unlock()
...
func (a *Engine) startup(ctx context.Context) {
	a.ctx = ctx
	...
	// Engine.RefreshModelFacts. Installed eagerly too, because the context meter
	...
	defer debuglog.Block("Engine.startup")()
...
func (a *Engine) openAtRememberedProject() bool {
...
// somewhere ordinary.
func (a *Engine) openAtRememberedDesk() {
	desk := mode.Default
...
func (a *Engine) outputSubdir() string { return a.outputSubdirOf(a.cur()) }
...
func (a *Engine) outputSubdirOf(conv *conversation) string {
...
// while a.cfg follows at the end of applyConfig. Reading both off the Engine gave
// "focused" paired with the previous project's path.
func (a *Engine) focusedProjectRoot(root string) string {
	if !a.projectFocused {
		return ""
	}
...
func (a *Engine) beginTurn(sessionID string) error {
	a.turnMu.Lock()
...
// already refused.
func (a *Engine) armTurnCancel(sessionID string, ctx context.Context, cancel context.CancelFunc) (stopNow bool) {
	a.turnMu.Lock()
	defer a.turnMu.Unlock()
...
func (a *Engine) disarmTurnCancel(sessionID string) {
	a.turnMu.Lock()
	defer a.turnMu.Unlock()
...
func (a *Engine) endTurn(sessionID string) {
	a.turnMu.Lock()
	if sessionID == "" {
		sessionID = a.cur().id
	}
...
func (a *Engine) anyTurnRunning() bool {
	a.turnMu.Lock()
	defer a.turnMu.Unlock()
	return len(a.turns) > 0
}

func (a *Engine) turnRunningIn(id string) bool {
	if id == "" {
		return false
	}
	a.turnMu.Lock()
...
// which chats are.
func (a *Engine) TurnInFlight() TurnStatus {
	open := a.cur().id
	a.turnMu.Lock()
	defer a.turnMu.Unlock()
...
func (a *Engine) SendMessage(text, to string) (TurnReply, error) {
...
func (a *Engine) runTurn(conv *conversation, text, to string) (SessionMessage, SessionMessage, error) {
	...
	// Off the engine's lifetime, not the window's (see Engine.lifeCtx): a turn
...
func (a *Engine) runAddressed(conv *conversation, ctx context.Context, agent, text string) (SessionMessage, SessionMessage, error) {
	now := time.Now().Format("15:04")
...
func (a *Engine) runAnswer(conv *conversation, ctx context.Context, text string) (SessionMessage, SessionMessage, error) {
	now := time.Now().Format("15:04")
	user := SessionMessage{Role: "user", Text: text, Time: now}
	dispatcher, ok := a.taskDispatcher()
	if !ok {
...
func (a *Engine) taskDispatcher() (subagent.Dispatcher, bool) {
...
func (a *Engine) Interject(text string) error {
	if a.cur().agent == nil {
...
func (a *Engine) ModelStatus() string {
...
func (a *Engine) contextWindowTokens() int {
...
func (a *Engine) GetModelInfo() ModelInfo {
	used := 0
	if a.cur().agent != nil {
...
func (a *Engine) modelSwitchResult() (ModelInfo, error) {
	if a.cur().chat == nil {
...
func (a *Engine) dialResult(conv *conversation) (ModelInfo, error) {
	a.turnMu.Lock()
...
func (a *Engine) preflightQueued(conv *conversation, next config.Config) {
...
func (a *Engine) emitPendingCheck(conv *conversation) {
	if conv == a.cur() {
...
// to arrive.
func (a *Engine) dialBase(conv *conversation) config.Config {
	a.turnMu.Lock()
	defer a.turnMu.Unlock()
...
func (a *Engine) CancelPendingModel() ModelInfo {
	conv := a.cur()
	a.turnMu.Lock()
	conv.pendingCfg = nil
	conv.pendingCheck, conv.pendingNote, conv.pendingProbe = "", "", ""
...
func (a *Engine) toolWeights(defs []model.ToolDefinition) []ContextTool {
	if len(defs) == 0 {
		return nil
	}
...
func (a *Engine) lastPromptUsage() (prompt, cached int) {
...
func (a *Engine) currentProjectStatus() ProjectStatus {
...
func (a *Engine) GetProjectStatus() ProjectStatus {
...
func (a *Engine) ClearProjectFocus() (ProjectStatus, error) {
	a.focusNone()
	a.startNewSession()
...
func (a *Engine) SupportedProviders() []string {
...
// than merely hide.
func (a *Engine) CustomProviders() []CustomProviderRow {
...
func (a *Engine) EnabledProviders() []string {
...
func (a *Engine) SetProviderEnabled(providerName string, enabled bool) ([]string, error) {
...
func (a *Engine) rememberQuotas(providerName string, quotas []model.Quota) {
	a.quotasMu.Lock()
...
func (a *Engine) forgetQuotas(providerName string) {
	a.quotasMu.Lock()
	defer a.quotasMu.Unlock()
...
func (a *Engine) ProviderAPIKeyURL(providerName string) string {
...
func (a *Engine) ProviderBaseURL(providerName string) string {
...
func (a *Engine) ProviderBaseURLIsCustom(providerName string) bool {
	canonical := model.NormalizeProvider(providerName)
...
func (a *Engine) SwitchModel(modelName string) (ModelInfo, error) {
...
func (a *Engine) RequiresAPIKey(providerName string) bool {
...
func (a *Engine) AcceptsAPIKey(providerName string) bool {
...
// would ever call.
func (a *Engine) SetProviderBaseURL(providerName, baseURL string) (ModelInfo, error) {
	canonical := model.NormalizeProvider(providerName)
	if _, ok := model.LookupProviderInfo(canonical); !ok {
...
func (a *Engine) RetryActiveProvider() ModelInfo {
	if a.cur().modelErr == nil {
		return a.GetModelInfo()
	}
	next := a.cur().cfg
...
func (a *Engine) SwitchProvider(provider string) (ModelInfo, error) {
	next := a.dialBase(a.cur())
...
// catalog default.
func (a *Engine) ProviderWireFormats(providerName string) []string {
	info, ok := model.LookupProviderInfo(model.NormalizeProvider(providerName))
...
func (a *Engine) SetProviderWireFormat(format string) (ModelInfo, error) {
	next := a.dialBase(a.cur())
...
func (a *Engine) SwitchApprovalMode(mode string) (ModelInfo, error) {
...
// never the point: it was how Engine.cfg got the new root, and the conversation
...
func (a *Engine) deskTools() *skill.Dispatcher {
...
func (a *Engine) chairProfile() *subagent.Profile {
	if a.cur().chair == "" {
		return nil
	}
...
func (a *Engine) sessionSkills(conv *conversation, sandboxRoot string) []skill.Skill {
	return []skill.Skill{
...
// out loud.
func (a *Engine) showConversation(conv *conversation) {
	outgoing := a.cur()
	a.convs.show(conv)
...
func (a *Engine) letGoOf(conv *conversation) {
