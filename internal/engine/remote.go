...
//  2. **No second gate.** Every endpoint here is a thin adapter over an Engine
...
func (a *Engine) StartMobileRemote() RemoteStatus {
	r := a.remote()
...
func (a *Engine) StopMobileRemote() RemoteStatus {
	a.remote().stop()
	return a.MobileRemoteStatus()
}

// MobileRemoteStatus is the panel's poll.
func (a *Engine) MobileRemoteStatus() RemoteStatus {
	r := a.remote()
	r.mu.Lock()
...
func (a *Engine) MobileRemoteQR() string {
	r := a.remote()
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.srv == nil || r.ip == nil {
...
func (a *Engine) PairedDevices() []RemoteDevice {
...
func (a *Engine) RevokeDevice(id string) error {
	db, err := a.database()
	if err != nil {
		return err
	}
...
// remote lazily builds the server holder. One per Engine.
func (a *Engine) remote() *remoteServer {
...
func (r *remoteServer) apiState(w http.ResponseWriter, _ *http.Request) {
	// Rule 2: both of these are the Engine methods the desktop calls. Nothing is
