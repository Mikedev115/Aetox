...
func closeDBOnCleanup(t *testing.T, a *Engine) {
	t.Helper()
	t.Cleanup(func() {
...
func newTestApp(t *testing.T, sandboxRoot string) *Engine {
	t.Helper()
	a := seed(&Engine{
		dbDir: t.TempDir(),
...
func TestListSessionsIsolatedByProject(t *testing.T) {
	...
	a := seed(&Engine{dbDir: dbDir, cfg: config.Config{SandboxRoot: rootA}}, newConversation())
	closeDBOnCleanup(t, a)
	a.startNewSession()
	a.appendTurn(a.cur(), SessionMessage{Role: "user", Text: "project A message"}, SessionMessage{Role: "agent", Text: "ok"})

	b := seed(&Engine{dbDir: dbDir, cfg: config.Config{SandboxRoot: rootB}}, newConversation())
