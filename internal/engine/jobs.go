...
// caused them.
func (a *Engine) maxToolRunID(conv *conversation) int64 {
	db, err := a.database()
	if err != nil {
		return 0
	}
...
func (a *Engine) recordJobs(conv *conversation, messageID int64, request, answer string, sinceToolRun int64, elapsed time.Duration) {
	if !learningEnabled() {
		return
	}
...
func (a *Engine) toolRunsSince(conv *conversation, mark int64) []toolRunRow {
	db, err := a.database()
	if err != nil {
		return nil
	}
...
func (a *Engine) RateTurn(messageID int64, verdict string) {
	switch verdict {
...
func (a *Engine) markTurnRedone(messageID int64) {
	db, err := a.database()
...
func (a *Engine) setJobOutcome(messageID int64, outcome, source string) {
	db, err := a.database()
	if err != nil || messageID == 0 {
		return
	}
...
func (a *Engine) TurnRating(messageID int64) string {
	db, err := a.database()
	if err != nil || messageID == 0 {
...
func (a *Engine) lastAgentMessageID(conv *conversation) int64 {
	sessionID := conv.id
	db, err := a.database()
	if err != nil || sessionID == "" {
		return 0
