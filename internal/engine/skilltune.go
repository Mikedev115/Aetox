...
// without a discovery scan and the same code backs the Engine reader below.
...
// skillMisfires is the Engine-facing read: it supplies the live skill set — the
...
func (a *Engine) skillMisfires() []skillMisfire {
	db, err := a.database()
	if err != nil {
		return nil
	}
