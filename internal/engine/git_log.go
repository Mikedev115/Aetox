...
func (a *Engine) GitLog(before string, limit int) GitLogPage {
...
func (a *Engine) GitCommitChanges(hash string) []GitFileChange {
	out := []GitFileChange{}
	root, ok := a.gitRoot()
...
func (a *Engine) GitCommitFileDiff(hash, path string) string {
	root, ok := a.gitRoot()
	if !ok {
		return ""
	}
