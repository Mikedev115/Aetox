...
func (a *Engine) GrantedComputerApps() []string {
	out := []string{}
	cfg, err := config.LoadPermissions()
	if err != nil {
		return out
...
func (a *Engine) RevokeComputerApp(name string) error {
	key := exeKey(name)
	if key == "" {
...
func (a *Engine) OpenComputerApps() []ComputerAppRow {
	out := []ComputerAppRow{}
...
func (a *Engine) AllowComputerApp(name string) error {
	key := exeKey(name)
	if key == "" {
		return fmt.Errorf("no program named")
	}
...
func (a *Engine) BrowseForComputerApp() (string, error) {
