...
func (a *Engine) WorkbenchTabsChanged(sessionID string, tabs []DeskTab) {
...
func (s *deskOpenSkill) open(path string) (skill.Output, error) {
	...
	// resolves against the project THAT chat is in — Engine.cfg is only the
...
func (a *Engine) openDeskTerminal(sessionID, command string) (shellName string, err error) {
	if a.ctx == nil {
