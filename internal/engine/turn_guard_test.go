...
func TestOpeningAChatInAnotherProjectOpensMidTurn(t *testing.T) {
	...
	// (Engine.cfg) and what a chat is actually running in (conversation.cfg).
	moved := t.TempDir()
...
// its engine let go, and reopening it rebuilt from Engine.cfg — the last model
...
func dialledChat(t *testing.T, a *Engine) *conversation {
	t.Helper()
	conv := a.cur()
