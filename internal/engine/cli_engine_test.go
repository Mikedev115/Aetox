...
func TestNothingIsRegisteredInThisBuild(t *testing.T) {
	...
	if st := (&Engine{}).ExternalEngineStatus("anthropic"); st.Ready || st.Detail != "" {
...
func TestCLIEngineTurnReportsWhyItCannotRun(t *testing.T) {
	...
	// ProviderReady is the screen's (desktop/providers.go) and asks this:
	// the engine's status is the whole of its answer for such a provider.
	if a.ExternalEngineStatus("fake-down").Ready {
		t.Error("ExternalEngineStatus said ready for an engine that is not")
	}
}
