...
// recorder collects what the Engine emitted. It holds a lock because the Engine has
...
// That distinction is the whole point of it: replacing `a.emit` on an Engine that
...
var bootRecorders sync.Map // *Engine -> *recorder

// captureEvents hands back the Engine's recorder, emptied. The real Wails emitter
// calls log.Fatalf when ctx is not Wails-bound, which is never in a unit test —
// see the `emit` field's comment in app.go — so an Engine that did not come from
// bootDeskApp still gets one installed here, before anything of its own runs.
func captureEvents(a *Engine) *recorder {
...
// Pins the recorder itself. The Engine emits from more than one goroutine —
