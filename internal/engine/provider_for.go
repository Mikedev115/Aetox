...
func (a *Engine) providerFor(cfg config.Config) func(string) (model.Provider, string, error) {
	return func(name string) (model.Provider, string, error) {
		...
		// The screen answers the default model — it holds the key the
		// discovery call needs (Screen.DefaultModel, §248) — as it does for
		// the provider picker.
		defModel := strings.TrimSpace(a.screenOf().DefaultModel(canonical, baseURL))
		if defModel == "" {
		...
		p, err := model.NewProvider(model.ProviderOptions{
			...
			SignedInEndpoint: a.screenOf().ProviderEndpoint(canonical),
		})
		if err != nil {
			return nil, "", err
		}
