...
func (a *Engine) cancelActiveReview() {
	activeReviewMu.Lock()
...
type appSessionReviewer struct{ app *Engine }
...
func (a *Engine) decidedMemoryLines(state string, limit int) []string {
	db, err := a.database()
	if err != nil {
		return nil
	}
...
func (a *Engine) runSessionReviewWith(ctx context.Context, reviewer sessionReviewer, sessionID string) (int, error) {
	if !learningEnabled() {
...
func (a *Engine) armSessionIdleReview(sessionID string) {
...
func (a *Engine) maybeReviewSession(sessionID string) {
	if !learningEnabled() || !sessionReviewAuto() || sessionID == "" {
		return
	}
...
func (a *Engine) RunSessionReview(sessionID string) (int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
	defer cancel()
...
func (a *Engine) SessionReviewAuto() bool { return sessionReviewAuto() }

// SetSessionReviewAuto persists the switch for Settings.
func (a *Engine) SetSessionReviewAuto(on bool) error {
