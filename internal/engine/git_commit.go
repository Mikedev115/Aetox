...
func (a *Engine) GitSuggestCommitMessage(files []string) (string, error) {
	root, ok := a.gitRoot()
	if !ok {
...
func (a *Engine) gitSplitBegin() (context.Context, uint64) {
	a.gitSplitMu.Lock()
	defer a.gitSplitMu.Unlock()
	if a.gitSplitCancel != nil {
		a.gitSplitCancel()
...
func (a *Engine) gitSplitSeated(seq uint64) bool {
	a.gitSplitMu.Lock()
	defer a.gitSplitMu.Unlock()
...
func (a *Engine) GitSuggestSplitCommits() ([]GitCommitGroup, error) {
	out := []GitCommitGroup{}
	tree, err := a.GitWorkingTree()
	if err != nil {
		return out, err
...
func (a *Engine) gitSplitWriteMessages(ctx context.Context, seq uint64, p model.Provider, modelName, recentLog string, tree []GitFileChange, diffs map[string]string, groups []GitCommitGroup) {
