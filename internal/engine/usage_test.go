...
func TestUsagePipelineEndToEnd(t *testing.T) {
	isolateUserDirs(t)
	a := seed(&Engine{cfg: config.Config{ModelProvider: "noop", ModelName: "usage-fake-model", SandboxRoot: t.TempDir()}, dbDir: t.TempDir()}, newConversation())
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
...
func TestUsageStatsPutsMoneyOnRowsItCanPrice(t *testing.T) {
	...
	a := seed(&Engine{cfg: config.Config{ModelProvider: "deepseek", ModelName: "deepseek-v4-flash"}, dbDir: t.TempDir()}, newConversation())
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
...
func TestSessionSpendKeepsSilentAboutWhatNobodyMeasured(t *testing.T) {
	...
	a := seed(&Engine{cfg: config.Config{ModelProvider: "lmstudio", ModelName: "some-local-model"}, dbDir: t.TempDir()}, newConversation())
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
...
func TestSubscriptionUsageIsCountedButNotPriced(t *testing.T) {
	isolateUserDirs(t)
	t.Setenv("AETOX_DATA_ROOT", t.TempDir())
	a := seed(&Engine{cfg: config.Config{ModelProvider: "codex", ModelName: "gpt-5.6-luna"}, dbDir: t.TempDir()}, newConversation())
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
...
func TestTheModelPriceSourceIsNamedAndDated(t *testing.T) {
	isolateUserDirs(t)
	t.Setenv("AETOX_DATA_ROOT", t.TempDir())
	a := seed(&Engine{cfg: config.Config{}, dbDir: t.TempDir()}, newConversation())
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
