...
func (a *Engine) recordToolRun(conv *conversation, run turn.ToolRun) {
	db, err := a.database()
	if err != nil {
...
// command log does.
func (a *Engine) recordChildParts(conv *conversation, parentRef string, parts []turn.TurnPart) {
...
func (a *Engine) withChildParts(conv *conversation, parts []turn.TurnPart) []turn.TurnPart {
	if conv == nil {
		return parts
	}
	a.toolHistoryMu.Lock()
