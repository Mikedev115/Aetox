...
// same Engine methods, use the same stored command, and wait the same way.
//
...
//
// Kept on the Engine rather than on the skill: the skill is rebuilt by every
...
func (a *Engine) engineStarting(id string) (time.Duration, bool) {
...
func (a *Engine) markEngineStarting(id string) {
	a.engineStartMu.Lock()
	defer a.engineStartMu.Unlock()
...
func (a *Engine) clearEngineStarting(id string) {
	a.engineStartMu.Lock()
	defer a.engineStartMu.Unlock()
	delete(a.engineStartedAt, id)
}
