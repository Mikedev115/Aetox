...
// appProposer adapts the Engine to learned.Proposer. The indirection exists so
...
// and reject.
type appProposer struct{ app *Engine }
...
func (a *Engine) memoryLedger(scopes []string) prompt.Ledger {
	db, err := a.database()
...
func (a *Engine) proposeLearned(p learned.Proposal) (learned.Result, error) {
	if !learningEnabled() {
...
func (a *Engine) queueMemoryProposal(p learned.Proposal, source, evidence string) (learned.Result, error) {
	db, err := a.database()
	if err != nil {
...
func (a *Engine) priorMemoryDecision(p learned.Proposal) (learned.Prior, bool, error) {
	db, err := a.database()
	if err != nil {
...
// another.
func (a *Engine) priorSkillDecision(skillName, op, before, body string) (learned.Prior, bool, error) {
	db, err := a.database()
	if err != nil {
		return learned.Prior{}, false, err
	}
...
func (a *Engine) ListPendingChanges() []PendingChange {
...
func (a *Engine) ListSkillProposals() []PendingChange {
...
// down, newest first.
func (a *Engine) ListDecidedSkillProposals(limit int) []PendingChange {
...
func (a *Engine) PendingSkillTuneCount() int { return a.countPending(kindSkill) }
...
func (a *Engine) ListDecidedChanges(limit int) []PendingChange {
	if limit <= 0 || limit > 200 {
		limit = 50
	}
	return a.queryChanges(
...
func (a *Engine) ListSystemIssues() []PendingChange {
...
// the shipped page.
func (a *Engine) ListDecidedIssues(limit int) []PendingChange {
	if limit <= 0 || limit > 200 {
		limit = 50
	}
	return a.queryChanges(
...
func (a *Engine) PendingChangeByID(id int64) PendingChange {
...
func (a *Engine) PendingLearnedCount() int {
...
func (a *Engine) PendingIssueCount() int {
	return a.countPending(kindIssue)
}

func (a *Engine) countPending(kind string) int {
	db, err := a.database()
	if err != nil {
		return 0
	}
...
func (a *Engine) ApprovePendingChange(id int64) error {
...
func (a *Engine) ApprovePendingChangeTo(id int64, scope string) error {
	db, err := a.database()
	if err != nil {
		return err
	}
...
func (a *Engine) RejectPendingChange(id int64) error {
	db, err := a.database()
	if err != nil {
		return err
	}
...
// twice.
func (a *Engine) MarkIssueReported(id int64) error {
	db, err := a.database()
	if err != nil {
		return err
	}
...
func (a *Engine) queryChanges(where string, args ...any) []PendingChange {
	out := []PendingChange{}
	db, err := a.database()
	if err != nil {
		return out
...
func (a *Engine) emitLearningChanged() {
...
// agent.
func (a *Engine) LearnedMemory(scope string) string {
...
// what comes back.
func (a *Engine) LearnedEntries(scope string) []string {
...
func (a *Engine) LearnedScopes() []string {
	out := learned.Scopes()
	if out == nil {
		return []string{}
	}
...
func (a *Engine) LearnedScopeInfos() []MemoryScopeInfo {
...
func (a *Engine) ForgetMemoryScope(scope string) error {
	scope = strings.TrimSpace(scope)
...
func (a *Engine) AdoptMemoryScope(scope, targetRoot string) error {
	scope = strings.TrimSpace(scope)
	if _, ok := learned.SplitProjectScope(scope); !ok {
...
func (a *Engine) SaveLearnedEntry(scope string, index int, text string) error {
...
func (a *Engine) AddLearnedEntry(scope, text string) error {
...
func (a *Engine) MoveLearnedEntry(fromScope, toScope string, index int) error {
...
func (a *Engine) MemoryFolderPath() (string, error) {
	dir, err := learned.Dir()
	if err != nil {
		return "", err
	}
...
func (a *Engine) SetLearningEnabled(on bool) error {
...
func (a *Engine) LearningEnabled() bool { return learningEnabled() }
...
func (a *Engine) SkillTuneAuto() bool { return skillTuneAuto() }

func (a *Engine) SetSkillTuneAuto(on bool) error {
	return config.UpdateModelPreference(func(pref *config.ModelPreference) error {
