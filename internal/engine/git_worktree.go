...
func (a *Engine) GitWorkingTree() ([]GitFileChange, error) {
	out := []GitFileChange{}
	root, ok := a.gitRoot()
	if !ok {
		return out, nil
...
func (a *Engine) GitFileDiff(path string) string {
	root, ok := a.gitRoot()
	if !ok {
		return ""
	}
...
func (a *Engine) gitRoot() (string, bool) {
	if !a.projectFocused {
		return "", false
	}
...
// refresh.
func (a *Engine) gitContext() (context.Context, context.CancelFunc) {
