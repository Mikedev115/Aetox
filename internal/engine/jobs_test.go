...
func readJobs(t *testing.T, a *Engine) []storedJob {
	t.Helper()
	db, err := a.database()
	if err != nil {
...
func newJobApp(t *testing.T) *Engine {
	t.Helper()
	...
	a := seed(&Engine{cfg: config.Config{}, dbDir: t.TempDir()}, &conversation{id: "20260804-120000.000"})
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
