...
// most of this package's tests construct a zero Engine, and two goroutines finding
// a nil field at the same time each build their own set and then disagree about
// which conversations are running.
func (a *Engine) goalRunSet() *goalRuns {
...
// work happened.
func (a *Engine) goalCheck(sessionID string) func(string) string {
...
func (a *Engine) StartPlanRun(sessionID string) PlanRunStart {
...
func (a *Engine) PausePlanRun(sessionID string) {
	sessionID = strings.TrimSpace(sessionID)
...
// wall.
func (a *Engine) ResumePlanRun(sessionID string) {
	sessionID = strings.TrimSpace(sessionID)
	run := a.goalRunSet().get(sessionID)
	if run == nil {
		return
...
func (a *Engine) SetPlanStepStop(sessionID string, n int, on bool) {
	sessionID = strings.TrimSpace(sessionID)
	plan, err := a.loadPlan(sessionID)
	if err != nil || plan == nil {
		return
...
func (a *Engine) PlanRunning(sessionID string) bool {
