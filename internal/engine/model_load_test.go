...
func loadWatcher(t *testing.T, resident bool) (*Engine, *conversation, chan ModelLoading, *httptest.Server) {
	t.Helper()
	...
	a := seed(&Engine{ctx: context.Background(), dbDir: t.TempDir()}, conv)
	t.Cleanup(func() {
		if a.db != nil {
			_ = a.db.Close()
		}
