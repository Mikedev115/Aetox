...
- Phase 6: §25 อนุมัติแล้ว 2026-07-26 และรวมเข้า **§44** — สเปกระดับลงมือ
  (โปรไฟล์ซับเอเจน, `task`, 3 ช่องที่ต้องแก้ในไฟล์เดิม, ลำดับ build)
  · หมายเหตุ: ระหว่างทางมีการสร้าง "เอเจนหลักให้เลือก" ขึ้นมาแล้ว **ตัดออก** —
  เมนคือผู้ช่วยตัวเดียว ไม่ได้เลือกจากลิสต์ (§44.0) Phase 6 เหลือเฉพาะซับเอเจน
...
## Phase 4 — ชุดคำสั่ง (เดิมชื่อ "Commands" → "พรอมต์สำเร็จรูป")
...
**ฝังตัวอย่าง 5 อันมากับ binary** ด้วย `go:embed` — เพราะหน้าที่เปิดมา
แล้วว่างเปล่าคือสาเหตุที่เกือบถูกลบทิ้ง ไฟล์ของผู้ใช้ชื่อชนกับของที่ฝังมา
จะชนะ (แก้ = ก๊อปแล้วแก้ ไม่ต้องสู้กับแอป)

**เปลี่ยนชื่อไทยอีกรอบ 2026-07-25: "ชุดคำสั่ง"** (owner) — เฉพาะคำที่ผู้ใช้เห็น
ฝั่งอังกฤษยังเป็น "Prompt presets" และชื่อในโค้ด/โฟลเดอร์ยังเป็น `preset`/
`prompts/` เหมือนเดิม ตัวอย่างเพิ่มเป็น 8 อัน (ARCHITECTURE.md §35, §37)
