...
type Chromium struct {
	...
	// AETOX PATCH: when a caller installs its own error callback via
	// SetErrorCallback, that callback owns the recovery decision — errorCallback
	// must NOT force os.Exit(1) behind its back. Upstream always exits, which
	// turns any single embedded WebView2's transient failure (a browser tab
	// hitting ERROR_INVALID_STATE from RTSS DLL injection, GPU driver hiccups,
...
func (e *Chromium) SetErrorCallback(callback func(error)) {
	if callback != nil {
		...
		e.customErrorCallback = true // AETOX PATCH: see errorCallback
	}
}
...
func (e *Chromium) Embed(hwnd uintptr) bool {
	...
	// AETOX PATCH: a controller-creation failure (e.g. ERROR_INVALID_STATE)
	// unblocks the loop above via inited but leaves no usable webview — report
	// failure so the caller tears the window down instead of using a nil webview.
...
func (e *Chromium) CreateCoreWebView2ControllerCompleted(res uintptr, controller *ICoreWebView2Controller) uintptr {
	if int32(res) < 0 {
		...
		// AETOX PATCH: upstream assumed errorCallback os.Exit'd here. With a
		// custom callback it returns, and `controller` is nil on failure — so we
		// must bail before the AddRef below nil-derefs. This is the exact path
		// ERROR_INVALID_STATE takes. Mark the embed failed and unblock Embed's
		// message loop (via inited) so Embed returns false and the caller cleans
		// up the window instead of navigating a nil webview.
