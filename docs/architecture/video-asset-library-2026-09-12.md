...
ไฟล์: `internal/assetlib/` (assetlib.go, probe.go, ทดสอบ) · `internal/engine/studio_library.go` (binding + เครื่องมือ `asset_find` + แถว readiness — เขียนที่ `desktop/` แล้วย้ายตาม §248 ตอน landing 12 ก.ย.: คลังอยู่ที่เครื่องยนต์ ส่วน `/aetox-shelf/` บนจอเป็น proxy ไป `/shelf/` ของเครื่องยนต์ผ่าน `rpc.FileProxy` การ reveal เป็นคู่แฝด `StudioAssetPath`/`RevealStudioAsset`)
