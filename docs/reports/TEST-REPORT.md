...
> Grouping: the 5 modules match the split discussed for [ARCHITECTURE.md §10](../internal/ARCHITECTURE.md#10-decision--agent-orchestrator-layer-proposed-approved-2026-07-21) (model management / model-control layer / orchestrator / UI-CLI / desktop app). A 6th "shared/cross-cutting" bucket is added for packages that don't cleanly belong to any one of the 5 — noted explicitly rather than forced into a module they don't fit.
...
## Checks that are not tests — vet, lint, race (เขียน 2026-08-29 · decision 2026-08-18, [docs/internal/DECISIONS.md §141](../internal/DECISIONS.md))
...
|---|---|---|
| `go vet ./...` | `verify.sh` stage `vet` · job `windows` ใน [ci.yml](../../.github/workflows/ci.yml) | ✅ gate |
| `golangci-lint run ./...` | `verify.sh` stage `lint` · job `windows` ใน ci.yml (gate) | ✅ gate |
| `golangci-lint ... --enable=gosec` | `verify.sh` (พิมพ์บรรทัดสรุป) · job `windows` | ❌ report เท่านั้น |
| `go test -race` | `verify.sh` stage `race` (ข้ามถ้าไม่มี C compiler) | ✅ ใน verify.sh เท่านั้น — ไม่มีใน CI อีกตั้งแต่ถอด job `unix` ออก 18 ก.ย. 2026 |

**ลินเตอร์ตัวไหนเปิด และตัวไหนยังไม่เปิดเพราะอะไร อยู่ที่ [DECISIONS.md §141](../internal/DECISIONS.md) ที่เดียว**
...
**สถานะ: ไม่มีปัญหา.** ครอบคลุม provider factory, bootstrap, thinking-level normalization, และ catalog ทั้งสองตัว (ดู [ARCHITECTURE.md §6.3](../internal/ARCHITECTURE.md) — สอง catalog นี้ยังไม่ได้ diff ว่าซ้ำกันแค่ไหน แต่ทั้งคู่มีเทสของตัวเองแยกกัน ไม่ใช่ปัญหาเรื่อง test coverage)
...
**สถานะ: ผ่าน แต่ขอบเขตแคบตามที่ตั้งใจ.** เทสคุมแค่ `Spawn`/`Get`/`Stop`/`List` ของ package ที่เพิ่งสร้างในเซสชันนี้ — ไม่มี integration test เพราะยังไม่มี front end ไหนเรียกใช้จริง (ดู [ARCHITECTURE.md §10](../internal/ARCHITECTURE.md)) ไม่ใช่ gap ที่ต้องปิดตอนนี้ เพราะ package เองก็ยังไม่ถูก wire เข้าใช้งาน
