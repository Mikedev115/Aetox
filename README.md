...
## ขยายศักยภาพให้โมเดล — โมเดลตาบอดก็เห็นได้ มือด้วนก็ทำงานได้

นี่คือหัวใจของ **Architecture > Parameters** แบบจับต้องได้:
โมเดล 9B ที่รันบนเครื่องคุณ อ่านภาพไม่ได้ ดูวิดีโอไม่ได้ แตะหน้าจอไม่ได้ —
...
| แตะเครื่องจริงไม่ได้ | `computer` (เมาส์ คีย์บอร์ด screenshot) | เห็นจอ → คิด → คลิก ครบลูป โดยผู้ใช้อนุมัติทุกครั้ง |

ลูปเต็มของโมเดลที่ไม่มีตา: `computer screenshot` → `image_ocr` อ่านจอ → ตัดสินใจ → `computer click/type`
ทุก action ของ `computer` ต้องผ่านการอนุมัติจากผู้ใช้ — อันตรายระดับเดียวกับ `shell` ก็เบรกระดับเดียวกับ `shell`
...
### 🔒 ความปลอดภัยของข้อมูลคุณ — ที่ทำแล้ว และที่ยังไม่ได้ทำ

Aetox ไม่ส่งข้อมูลอะไรออกจากเครื่องคุณนอกจากที่คุณสั่ง แต่ก็ควรบอกให้ชัดว่าตอนนี้เก็บของยังไง:

| เรื่อง | สถานะตอนนี้ |
|:---|:---|
| **ประวัติแชท / ไฟล์โปรเจกต์** | อยู่ในเครื่องคุณล้วนๆ ไม่มี server ของเราตรงกลาง |
| **API key** | เก็บเป็นไฟล์ในเครื่อง ตั้งสิทธิ์ให้เจ้าของอ่านได้คนเดียว — แต่ยังไม่ได้เข้ารหัส 🔜 ขั้นถัดไปย้ายเข้าที่เก็บรหัสของตัว OS (Windows DPAPI / macOS Keychain) เพื่อให้โปรแกรมอื่นในเครื่องอ่านไม่ได้ |
| **คำสั่งที่ agent รัน** | รันในโฟลเดอร์โปรเจกต์ที่คุณเปิดเท่านั้น ออกนอกโฟลเดอร์ไม่ได้ และมีบันทึกไว้ทุกคำสั่ง |
...
| `computer` | ควบคุมเครื่องจริง: เมาส์ คีย์บอร์ด screenshot (ขออนุมัติทุกครั้ง) |
...
| **API key ในที่เก็บรหัสของ OS** | 🔜 ตอนนี้เป็นไฟล์สิทธิ์เจ้าของ — ถัดไป Windows DPAPI / macOS Keychain |
| **Knowledge Base** | 🔜 Obsidian + codebase + web |
