...
**ในกลุ่มเครื่องมือ AI ช่วยเขียนโค้ดและผู้ช่วยทั่วไป Aetox เบาที่สุด** — ไม่ใช่คำโฆษณา เราลงตัวอื่นไว้บนเครื่องเดียวกันแล้ววัดพร้อมกัน (Windows 11)

**พื้นที่ที่กินในเครื่องหลังติดตั้ง** — น้อยกว่าดีกว่า
...
ทั้งสามตัวที่เอามาเทียบทำงานเดียวกับ Aetox — AI ที่ช่วยเขียนโค้ดและใช้เครื่องมือแทนคุณได้ ต่างกันตรงที่**มันเป็นแค่หน้าจอพิมพ์คำสั่ง ไม่มี UI ให้กดสักปุ่ม** ส่วน Aetox มีหน้าต่างแชท เบราว์เซอร์ในตัว อ่านรูป ถอดเสียง ครบอยู่ข้างใน — แล้วยัง**เล็กกว่า Claude Code 7 เท่า**

> กราฟนี้เทียบเฉพาะเครื่องมือที่ทำงานเดียวกัน ไม่เอา IDE หรือ editor มาปนเพื่อให้ตัวเลขดูดี — คนละงานกัน เทียบแล้วไม่มีความหมาย

**เวลาเปิดและ RAM** — สามตัวข้างบนไม่มีหน้าจอแอป จับเวลาเปิดไม่ได้ เลยต้องหาไม้บรรทัดคนละแบบ: **Zed** แอปเนทีฟเขียนด้วย Rust ที่ขึ้นชื่อว่าเบาและเร็วที่สุดกลุ่มหนึ่ง วัดพร้อมกันบนเครื่องเดียวกัน สภาพเดียวกัน

| วัดอะไร | Aetox | Zed |
|:---|:---|:---|
| **เปิดครั้งแรก** (เครื่องยังไม่แคช) | **1.77 วิ** | 2.12 วิ |
| **เปิดครั้งถัดๆ ไป** | **0.53 วิ** | 0.53 วิ |
| **RAM ที่จองไว้จริง** (private) | **252 MB** | 471 MB |
| **จำนวน process ที่เปิดค้างไว้** | **7** | 4 |
| **พื้นที่ในเครื่อง** | **33 MB** | 419 MB |

เปิดเร็วเสมอ Zed ที่ราวครึ่งวินาที ใช้ RAM ราวครึ่งเดียว และกินพื้นที่ราว 1 ใน 12

**ความเสถียร** — `go test ./...` ผ่าน **745/745** ครอบคลุม 24 package, 0 fail · ฝั่งหน้าจออีก **83/83**

> **Zed ไม่ใช่คู่แข่ง และ Aetox ไม่ได้มาแทน editor ของคุณ** — เอามาเทียบเพราะมันเป็นมาตรฐานที่โหด
> ถ้าเบากว่าของที่ขึ้นชื่อว่าเบาได้ ตัวเลขถึงจะมีความหมาย · เครื่องมือ AI แบบพิมพ์คำสั่งอย่าง
> Claude Code / Codex CLI / OpenCode ไม่มีหน้าจอแอปให้จับเวลาเปิด จึงเทียบได้แค่ขนาดตามกราฟข้างบน
...
```
Aetox  0.13 มิลลิวินาที  ต่อการคุย 1 ครั้ง (tools แกนกลาง 20 ตัว ~9.9 KB)
```

เท่ากับ **1 ใน 8,000 ของวินาที** — เวลาที่คุณรอคือเวลาที่โมเดลคิด ไม่ใช่เวลาที่แอปเตรียมของ

> วัดเฉพาะขั้นตอนนี้ ไม่ได้รวมทั้งเส้นทาง — 200 รอบ ทำซ้ำ 3 ครั้ง ได้ 122/122/128 ไมโครวินาที
...
> เบาไม่ได้แปลว่าตัดฟีเจอร์ทิ้ง — 26 tools, 13 providers, sub-agent delegation ครบ ในตัวติดตั้ง 13 MB
