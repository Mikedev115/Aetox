...
- **แผงงานสี่ห้อง ที่เอเจนลงมือในห้องเดียวกับที่คุณมองอยู่** — สไลด์ เบราว์เซอร์ ไฟล์
  และเทอร์มินัล อยู่ในหน้าต่างเดียวกับบทสนทนา นี่ไม่ใช่แชทที่ส่งไฟล์กลับมาให้ตอนจบ
  แต่เป็นที่ทำงานที่คุณเห็นทุกขั้นตอนขณะมันกำลังทำ และเข้าไปแตะเองได้ทุกเมื่อ
  ประตูโค้ดมีห้องที่ห้าเพิ่มมาคือ **Git** ซึ่งกางไฟล์ที่ยังไม่ commit พร้อม diff รายไฟล์
- **ทำเด็คนำเสนอได้** — ตัวเด็คคือไฟล์ `.html` เดียวจบ ซึ่งเป็นรูปแบบที่โมเดล
  เขียนได้ดีจริง ส่วนการแปลงมันไปเป็นอย่างอื่นเป็นงานของแอป ไม่ใช่งานของโมเดล เด็คเปิดในห้องสไลด์
  ตั้งแต่วินาทีที่เขียนเสร็จ การส่งออกจึงเป็นปุ่มบนจอที่คุณดูอยู่แล้ว ไม่ใช่ขั้นตอนที่สอง —
  ได้ `.pdf` รูปภาพ หรือ `.pptx` สไลด์วางบนกล่อง 1280x720
  ซึ่งเท่ากับหน้าจอกว้างของ PowerPoint พอดี
- **ระบบควบคุมเบราว์เซอร์เป็นของเราเอง** — ตัวหน้าต่างคือ WebView2 ที่ประกอบเข้ามาในแอปจริง
  มีช่องที่อยู่ ปุ่มย้อนกลับ/ไปหน้า DevTools และชุดขนาดอุปกรณ์แปดแบบที่ปรับขนาดหน้าต่างจริง
  ให้ CSS media query ทำงานจริง ส่วนที่เราเขียนคือชั้นบนของมัน: การอ่านหนึ่งครั้งติดหมายเลข
  ให้ทุกองค์ประกอบที่กดได้บนหน้า แล้วคำสั่งคลิกกับพิมพ์เล็งไปที่หมายเลขนั้น โมเดลที่มองภาพไม่เห็น
  จึงกดถูกปุ่ม ไม่ต้องใช้โมเดลที่มองเห็น และไม่ต้องเดาพิกัด ([ดูภาพตัวอย่าง](#เอาไปทำอะไรได้บ้าง))
- **ทำเว็บและทำระบบได้จริง ไม่ใช่แค่พ่นโค้ดใส่ช่องแชท** — ชี้ไปที่โฟลเดอร์แล้วได้แผงไฟล์
  เอดิเตอร์ Monaco เทอร์มินัล PTY จริงไม่จำกัดแท็บ `git` `grep` และ `glob` ทั้งต้นไม้
  พร้อม `diagnostics` กับ `symbol` ที่หนุนด้วย language server ซึ่งแอปติดตั้งให้เอง
  ตอนใช้ครั้งแรก มันเขียนแล้วเปิดดูผลในเบราว์เซอร์ห้องข้าง ๆ ได้ในรอบเดียว
- **ความสามารถมาจากตัวแอป ไม่ได้มาจากพารามิเตอร์โมเดล** — OCR ไทย/อังกฤษ และการถอดเสียง
  ...
  จึงทำงานเหล่านี้ได้เท่าโมเดลตัวใหญ่ ไม่ใช่โมเดลทำเอง แอปทำให้
...
**"Virus detected" หรือชื่อ `Program:Win32/Wacapew.C!ml`** — เป็นคำตัดสินจากโมเดล
machine learning บนคลาวด์ ไม่ใช่ลายเซ็น แปลว่าไม่มีใครวิเคราะห์ไฟล์นี้แล้วชี้ว่าอันตราย
มันยิงเพราะตัวติดตั้งไม่ได้เซ็น ยังใหม่ ไม่มีชื่อเสียง และตอนติดตั้งมันโหลด Tesseract,
poppler, ffmpeg กับโมเดลเสียงมาด้วย ซึ่งเป็นรูปร่างเดียวกับดาวน์โหลดเดอร์
ทั้งสี่ตัวปักหมุดไว้ที่ release tag ที่แก้ไม่ได้ และทุกไฟล์ถูกตรวจ SHA256 ที่ฝังไว้ใน
[สคริปต์ตัวติดตั้ง](desktop/build/windows/installer/project.nsi) ก่อนใช้งาน ถ้าค่าไม่ตรงมันจะข้ามตัวนั้นไป
ไม่ใช่ใช้ต่อ บิลด์นี้ส่งให้ Microsoft ตรวจเป็น false positive แล้ว แต่รุ่นใหม่แฮชเปลี่ยน
...
**ทำเด็คนำเสนอ** บอกหัวข้อกับสิ่งที่อยากได้ แล้วเด็คจะกลับมาเป็นไฟล์
`.html` เดียวจบ ที่เป็น HTML เพราะตั้งใจ: มันคือรูปแบบที่โมเดลเขียนได้ดี เป็นไฟล์เดียวที่เป็นของคุณ
และแก้ด้วยมือได้ และเปิดได้บนเครื่องไหนก็ตามที่มีเบราว์เซอร์ ส่วนการแปลงมันไปเป็นไฟล์นำเสนอ
เป็นงานของแอป ไม่ใช่งานของโมเดล

เด็คถือว่าส่งมอบแล้วตั้งแต่ตอนที่มันเปิดในห้องสไลด์ ไม่ใช่ตอนส่งออก คุณเลื่อนดูทีละหน้า
และนำเสนอเต็มจอได้จากตรงนั้น และแถบส่งออกก็อยู่บนจอเดียวกัน ส่วน `.pdf` คือตัวไฟล์เด็คเอง
ผ่านตัวเรนเดอร์ที่วาดมันบนจอ หน้าตาจึงเหมือนสิ่งที่คุณเพิ่งดูอยู่เป๊ะ ๆ

โน้ตผู้บรรยายติดไปด้วยทุกทาง ส่วนการส่งออกเป็นรูปจะได้หนึ่งไฟล์ต่อหนึ่งสไลด์
ลงในโฟลเดอร์ของมันเอง ตั้งชื่อ `01` `02` เด็คสิบหน้าจึงเรียงถูกทุกที่ กล่องของสไลด์คือ 1280x720
ซึ่งเท่ากับ 13.333 x 7.5 นิ้ว ที่ 96dpi พอดี คือหน้าจอกว้างของ PowerPoint เป๊ะ ๆ
เด็คจึงไปเปิดต่อในนั้นได้โดยไม่เพี้ยน
...
แกนหลักเข้าที่แล้ว [บันทึกการปล่อยรุ่น](docs/release-notes/v1.5.7.md) ·
