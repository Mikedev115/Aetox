...
มันอ่านและเขียนไฟล์จริง รันคำสั่งจริงในเชลล์จริง ขับเบราว์เซอร์จริงที่คุณนั่งดูได้
และส่งไฟล์ `.docx` / `.xlsx` / `.pptx` ที่ Word, Excel และ PowerPoint เปิดได้จริงกลับมา
...
และ macOS ใน CI แล้ว แต่ช่องเบราว์เซอร์ยังเป็นตัวหลอกและยังแพ็กไม่เสร็จ 1.0.0
ต้องออกครบทั้งสามแพลตฟอร์ม ไม่งั้นไม่ใช่ 1.0.0 ดู [PLATFORM-SUPPORT.md](PLATFORM-SUPPORT.md)
...
**ส่งงานให้ช่างเฉพาะทาง** พิมพ์ `@doc` `@sheet` `@deck` `@github` หรือ `@automation`
...
มีเอเจนมาให้ห้าคน — `deck` `doc` `sheet` `github` `automation` —
...
**11 ผู้ให้บริการในแอป** — OpenAI · Anthropic · Gemini · DeepSeek · Qwen · Z.ai · OpenRouter ·
Codex · LM Studio · Ollama · และ `aetox` ที่มีในตัว OpenRouter กับ Codex ใช้การลงชื่อเข้าใช้
ที่เหลือใช้ API key หรือที่อยู่เซิร์ฟเวอร์ในเครื่อง ทะเบียนของเครื่องยนต์มี 18 ราย
ส่วนที่เหลือไม่ได้ถูกวาดไว้ในหน้าต่าง
...
แกนหลักเข้าที่แล้ว [บันทึกการปล่อยรุ่น](docs/release-notes/v0.9.6.md) ·
[แผนไป 1.0.0](ROADMAP.md) · [สถาปัตยกรรม](ARCHITECTURE.md)
...
โค้ดอยู่ภายใต้ [Apache-2.0](LICENSE): ใช้ แก้ แจกจ่าย และขายได้อย่างอิสระ
รวมถึงในงานที่ไม่เปิดซอร์ส ขอสองอย่างเป็นการตอบแทน — เก็บไฟล์ [NOTICE](NOTICE)
และประกาศลิขสิทธิ์ไว้ และบอกว่าคุณแก้ไฟล์ไหนบ้าง

ชื่อ **"Aetox"** และโลโก้เป็นเครื่องหมายการค้า และ**ไม่ได้**อยู่ในสัญญาอนุญาต
(Apache-2.0 ข้อ 6) fork ได้เต็มที่ สิ่งที่คุณเอาไปแจกต่อใช้ชื่อของคุณเอง
จะได้ไม่มีใครสับสนว่าตัวไหนคือต้นฉบับ รุ่น 0.7.1 และก่อนหน้าเป็น MIT
และรุ่นเหล่านั้นเป็น MIT ตลอดไป
...
<p align="center">
  © 2026 ชยพล พรมสะวะนา · <a href="LICENSE">Apache-2.0</a> · "Aetox" เป็นเครื่องหมายการค้า ไม่ได้อยู่ในสัญญาอนุญาต
</p>
