...
- **24 ผู้ให้บริการโมเดล** — ทั้งคลาวด์ (OpenAI, Anthropic, Gemini, DeepSeek, Groq ฯลฯ)
...
| **วิธีที่ผู้ช่วยทำงาน** | `ask_user` `calc` `desk` *(open · list · close · focus)* `memory` `plan_mode` `plan` *(write · amend · read · step · report)* `plugin_install` `session_search` `skill_view` `skills_list` `task` *(start · collect · answer · message · plan)* `time` `todo_write` |
...
**24 ผู้ให้บริการ และหน้าต่างแสดงครบทุกราย** — OpenAI · OpenAI-compatible (ปลายทางของคุณเอง) ·
...
แกนหลักเข้าที่แล้ว [บันทึกการปล่อยรุ่น](docs/release-notes/v1.7.5.md) ·
[แผนงาน](ROADMAP.md) · [สถาปัตยกรรม](ARCHITECTURE.md)

สามอย่างที่มันทำได้วันนี้และน่ารู้ไว้:

- **มันแนะนำตัวเอง และบริษัทมียศสามชั้น** ระหว่างเลือกภาษากับต่อสมอง ทัวร์เก้าฉาก (รู้จักกับ Aetox)
  พาดูสี่ห้องของหน้าต่าง สองหัว ความจำที่ถูก "เสนอ" ไม่ใช่เขียนเอง ข้อมูลทุกไบต์อยู่ที่ไหน
  และทีมส่งงานกันยังไง — ดูซ้ำได้ที่ ตั้งค่า › เกี่ยวกับ หัวทั้งสอง (ผู้ช่วย กับ โค้ด) พนักงาน
  และลูกมือทุกตัวใช้หน้าโปรไฟล์แบบเดียวกัน: หน้าที่มียศอยู่มุม ชื่อที่เปลี่ยนได้ สามไฟล์ที่ตอบว่า
  เป็นใคร / คิดยังไง / โต๊ะทำอะไร และไฟล์ความจำของตัวเอง — สิ่งที่งานสอน*มัน* แยกจากสิ่งที่มัน
  รู้เกี่ยวกับคุณ
- **คอนโซลเป็นจอที่สาม และปุ่มปรับได้ระหว่างเทิร์นกำลังวิ่ง** `aetox chat` นั่งโต๊ะโค้ดบนเครื่องยนต์
  ตัวเดียวกับหน้าต่าง มี `--report` เป็น JSON หนึ่งบรรทัดต่อเทิร์น ระดับคิดกับโหมดอนุมัติเปลี่ยน
  กลางเทิร์นได้แล้ว — รอบถัดไปของลูปเครื่องมืออ่านค่าใหม่ และลูกมือทุกตัวที่จ้างหลังกดก็ด้วย
  วางแผนได้ยินเสียงเดียว (พรอมต์เหลือครึ่ง) และปุ่มของแผนส่งสกิลเล่มเดียวให้มุ่งเป้าพร้อม
  ข้อความแรกของรัน ไม่สร้างเครื่องยนต์ใหม่ ช่วงงานเครื่องมือพับเมื่อจบ ไม่ใช่ทุกครั้งที่เงียบ
- **ชั้นสกิลอ่านสกิลของตัวเองได้ และโตขึ้น** `internal/skilllint` กับ `aetox skill lint` อ่านสกิล
  ก่อนคนตัดสิน และสกิลที่แอปร่างเองทุกใบผ่านด่านเดียวกัน ของใหม่บนชั้น: `aetox-security`
  (เส้นทางโจมตีที่บรรทัด ไม่งั้นคือคำแนะนำ) `aetox-performance` (ตัวเลขที่บรรทัด ผ่าน noise)
  `aetox-orient` แก่นแปดตัวของ obra/superpowers, motion กับ web-3d, idea-to-architecture,
  code review เขียนใหม่ และ `codebase design` ที่จับ tell ของ UI ที่เขียนตามนิสัย ห้องความสามารถ
  ถือ MCP · สกิล · เครื่องมือในตัว · การใช้คอมพิวเตอร์ · การเชื่อมต่อ · ชุดคำสั่ง ทั้งก้อน

**ต่อไป** — โซ่ผู้ให้บริการที่สลับบัญชีเองเมื่อหน้าต่างแพลนหมด · โปรแกรมเอเจนภายนอกเป็นเครื่องยนต์
(§258) · วัด RAM รอบสภาพเดียวกันทุกแอปในฉากสุดท้ายของทัวร์ (ก่อนนั้นแถวบอกเองว่าตัวไหนวัดเอง
ตัวไหนจากรายงานสาธารณะ)
