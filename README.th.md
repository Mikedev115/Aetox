...
**ส่งงานให้ช่างเฉพาะทาง** พิมพ์ `@doc` `@sheet` `@github` `@automation` หรือ `@deepresearch`
...
| **มีห้องอะไร** | ผู้ช่วย · ความสามารถ · โปรเจกต์ · เอเจนเฉพาะทาง · งานวิดีโอ · ผลงาน | โค้ด |
| **แผงขวามือ** | มี | มี |
...
**28 เครื่องมือถึงมือโมเดลบนเครื่องที่เพิ่งติดตั้ง** เซสชันผู้ช่วยปกติหยิบน้อยกว่านั้น
เพราะโต๊ะเป็นตัวตัดชุดให้แคบลง ทั้งหมดกินราว 7,527 โทเคนในทุกคำขอ ก่อนคุณจะพิมพ์อะไรด้วยซ้ำ
เทียบกับเพดาน 10,400 โทเคนและ 48 เครื่องมือ ที่มีเทสต์บังคับไว้ สิบตัวในนั้นเป็นเครื่องมือ **ห่อ**
คือหนึ่งชื่อในบล็อกโดยมีหลายคำกริยาอยู่ข้างใน ซึ่งเป็นเหตุผลที่รายการสั้นลงใน v1.5.15
โดยไม่มีความสามารถไหนหายไป
...
| **อ่านสื่อ** | `media_read` *(image · video · audio)* `pdf_read` |
...
| **วิธีที่ผู้ช่วยทำงาน** | `ask_user` `calc` `desk` *(open · list · close)* `memory` `plugin_install` `session_search` `skill_view` `skills_list` `task` *(start · collect · answer · plan)* `time` `todo_write` |
...
| เทสต์ฝั่ง Go | 2,479 ตัว ใน 43 แพ็กเกจ ไม่มีตัวไหนล้ม |
| เทสต์ฝั่งหน้าจอ | 1,054 ตัว ใน 107 ไฟล์ ไม่มีตัวไหนล้ม |
