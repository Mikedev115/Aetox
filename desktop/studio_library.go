...
func (a *App) StudioSetKind(id, kind string) error {
...
// RevealStudioAsset opens the folder holding one asset, with the file
// selected where the platform allows.
func (a *App) RevealStudioAsset(id string) error {
	...
	return a.reveal(lib.FullPath(*as), nil)
}
...
func (a *App) StudioLibraries() []StudioLibraryView {
...
func (a *App) runStudioScan(ctx context.Context, dir string, tools assetlib.FFTools) {
	defer func() {
		a.studio.mu.Lock()
...
func (a *App) RemoveStudioLibrary(id string) ([]StudioLibraryView, error) {
	s, root, err := studioStore()
	if err != nil {
...
// RevealStudioLibrary opens a shelf's folder in the file manager — the
