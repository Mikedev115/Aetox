...
func TestTheOrphanSweepSparesARunningTurnsTabs(t *testing.T) {
	app := hostWithTabs(t, "web-agent-1", []string{"web-agent-1", "web-agent-2"},
		"web-agent-1", "web-agent-2", "web-3")
	app.api = engineWith{API: app.api, turnRunning: true}
...
// it. The busy signal asks after the tab on every single tool call (the
// engine, through Screen.AgentTab), so if it asked the same way, it would eat
// the sentence telling the model its page was closed out from under it, and
// the model would be told instead that it had never opened one. A UI detail
// deleting a message meant for the agent.
func TestTheStampNeverEatsThePageWasClosedMessage(t *testing.T) {
	app := hostWithTabs(t, "web-agent-1", []string{"web-agent-1"}, "web-agent-1")
	app.BrowserClose("web-agent-1") // the user's × on the tab strip

	// The panel asks first, the way the engine does on every call in the turn.
	if got := (appScreen{app}).AgentTab(); got != "" {
		t.Errorf("AgentTab() = %q for a tab that is gone", got)
	}
