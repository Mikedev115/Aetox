...
var browserGuidance = map[string]string{
	...
	"open": "newTab=true keeps the page you are on and opens an extra one. The question is whether you are still going to need this page: gathering several sources to write from, or holding two side by side to compare, means keep them — open each in its own tab. A page you have already taken what you need from is finished, so let the next open replace it, and use `back` if you were wrong: re-opening a URL is not the same as going back, because a form you filled and a scroll position do not survive it.\n" +
		...
		"You do not need to see the page to drive it. Every click, type, key and scroll answers with what changed and what is under the pointer, and `read` gives you the words and every element's ref; that is how you steer. `capture` is for what was never text — a chart, a canvas, a map, a layout you suspect is wrong — and not for checking that a move landed. A picture after every move is the one habit that makes a browser session cost ten times what it should.",
	...
	"capture": "Use this only when `read` cannot answer, because the answer was never in the text: a chart, a canvas, a map, a rendered document, or a layout you suspect is wrong. Read first, photograph second — a picture costs far more than the read it would have duplicated.\n" +
		...
		"`marks=true` draws each element's ref onto the picture and prints what every number is. Use it whenever the point of looking is to press something: you read a number off the page and click that ref, which is exact and survives the page moving — a pixel measured by eye is neither. It marks what is in the frame, so scroll and photograph again for the rest, and it is viewport-only (a full-page picture is shrunk until the labels cannot be read).\n" +
		"Without marks the answer says how the picture's pixels map to the page: multiply a pixel you read off it by the ratio given and you have the x,y to click, hover or drag at. That is the aim of last resort, for what has no ref at all: a cell in a painted sheet, a shape on a canvas. Aim only from a viewport capture — a full one's y is a document offset, not a place on screen.\n" +
	...
	"click": "A ref belongs to the page it was read from AND to the read that produced it: the next read renumbers everything, and a filtered read tags only its own matches, so a short list carries ref 1 upward whatever the full page holds. It also goes stale the moment the page changes, which a click often does. Read, act, read again — or aim by text instead: `find` presses the one element whose text contains it — a control first, then any visible text — waits up to three seconds for it to appear, and refuses with a list when there are several, so nothing stale is ever pressed.\n" +
		"A feed is the case where several is the normal answer: every post repeats the same 'Like', 'Comment', 'Share', so `find` on that word matches one per post and rightly refuses rather than guess whose. That refusal is not a dead end — it means the word alone cannot say which one, not that none can be reached. `read {filter:\"Like\"}` tags every match with its own ref in page order, and the surrounding text `read` returns with it (the author, the caption) is how you tell them apart; click the specific ref, not the word again.\n" +
