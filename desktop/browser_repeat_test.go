...
func TestCaptureOfADeckNamesTheRendererItIsNot(t *testing.T) {
	...
	note := app.deckRendererNote(engine.FileURLForPath(deckPath))
	if note == "" {
	...
	if got := app.deckRendererNote(engine.FileURLForPath(plain)); got != "" {
	...
	if got := app.deckRendererNote(engine.FileURLForPath(filepath.Join(dir, "gone.html"))); got != "" {
