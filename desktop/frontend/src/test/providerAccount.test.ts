  ...
  it('renders circular rings beside percentages in compact mode as standard', () => {
    ...
    const { container } = render(ProviderAccount, {
      account: account({
        ...
        expectsQuota: true, quotaKnown: true, quotaFetched: true,
        quotas: [
          { window: '5h', remainingPercent: 82, resetAt: new Date(Date.now() + 4 * 3600_000).toISOString(), observedAt: now },
          { window: 'week', remainingPercent: 64, resetAt: new Date(Date.now() + 4 * 86400_000).toISOString(), observedAt: now },
        ],
      }),
      compact: true,
    })
