<script lang="ts">
  ...
  let page = $state(1)
  let result = $state<main.StudioAssetPage | null>(null)
  ...
  $effect(() => {
    ...
    void (async () => {
      ...
      try {
        const got = await StudioAssets(q as main.StudioAssetQuery)
        if (mine !== seq) return
        result = got
  ...
  const posterOf = (row: main.StudioAssetView) => row.thumb || thumbs[row.id] || ''
  const posterPending = (row: main.StudioAssetView) => !row.thumb && !(row.id in thumbs)
  ...
  let playing = $state('')
  function toggle(row: main.StudioAssetView) {
    if (!audio) {
      audio = new Audio()
  ...
  let hovered = $state('')
  function hoverIn(row: main.StudioAssetView) { if (row.playable && !AUDIO.has(row.ext)) hovered = row.id }
  ...
  function patch(id: string, change: Partial<main.StudioAssetView>) {
    if (!result) return
    result = { ...result, rows: result.rows.map((r) => (r.id === id ? { ...r, ...change } : r)) } as main.StudioAssetPage
  }
  async function setKind(row: main.StudioAssetView, kind: string) {
    correctError = ''
    try {
  ...
  async function setHidden(row: main.StudioAssetView, hidden: boolean) {
    correctError = ''
    try {
      ...
      if (hidden && !includeHidden && result) {
        ...
        result = { ...result, rows: result.rows.filter((r) => r.id !== row.id), total: result.total - 1 } as main.StudioAssetPage
      } else {
        patch(row.id, { hidden })
      }
    } catch (err) {
