<script lang="ts">
  ...
  // 0 language · 1 connect · 2 approval. Appearance and optional capability
  // downloads live in Settings, where they can be changed with context.
  let step = $state(0)
    ...
    {#key step}
      <div class="ob-screen">
        ...
        {:else if step === 2}
          ...
          <div class="ob-links"><button class="ob-link" disabled={!!busy} onclick={() => { step = 1; intent = '' }}>{t('onboard.back')}</button></div>
        {/if}
      </div>
    {/key}
    {/if}

    <!-- Three decisions only: language, model connection and approval. -->
    {#if !tour}
    <div class="ob-dots">
      {#each [0, 1, 2] as i}<i class:on={step === i} class:past={step > i}></i>{/each}
    </div>
