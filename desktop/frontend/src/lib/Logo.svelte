<script lang="ts">
  let { size = 22, animate = true }: { size?: number; animate?: boolean } = $props()
</script>

<svg
  class="logo-mark" class:animate width={size} height={size} viewBox="0 0 804 762"
...
<style>
  .logo-mark { color: var(--text-primary); flex-shrink: 0; }
  .logo-mark.animate {
    animation: pop-in .45s cubic-bezier(.2, .9, .3, 1.3) both;
  }
  @keyframes pop-in {
