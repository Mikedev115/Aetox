<script lang="ts">
  ...
  let switches = $state<Record<string, engine.DelegateSettings>>({})
  ...
  async function load() {
    ...
    const next: Record<string, engine.DelegateSettings> = {}
  ...
  const teamLabel = (tm: engine.TeamCard) => tm.name
  ...
  function inReach(tm: engine.TeamCard): number {
  ...
  function reachOf(team: engine.TeamCard, name: string): { on: boolean; off: boolean } | null {
  ...
  async function toggleMember(team: engine.TeamCard, name: string, on: boolean) {
    if (busy) return
  ...
  function editTeam(tm: engine.TeamCard) {
    editing = {
