...
type computerSkill struct {
	app *App
	// session is whose chat is driving — a engine.Session, not the conversation,
...
func newComputerSkill(a *App, sess engine.Session) *computerSkill {
...
func (s *computerSkill) takeTheScreen(t reachTarget, doing string) error {
	...
	overlay.show()
	s.app.emitEvent("computer:driving", engine.SessionEvent[map[string]any]{
