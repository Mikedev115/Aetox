...
// window is — the browser tab, the mouse. Today the screen is this very App
...
//
// Not *App itself, on purpose. Every exported method of App is a Wails
...
// adapter, and App has no exported method the engine's contract would need.
...
// this window. Lazy, so an App built by struct literal has one without asking.
func (a *App) screenOf() Screen {
	if a.screen != nil {
		return a.screen
	}
	return appScreen{a}
