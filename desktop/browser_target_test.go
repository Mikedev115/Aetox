...
func TestEveryBrowserActionNamesAPageTheSameWay(t *testing.T) {
	...
	ref := engine.BrowserPageRef(title, url)
	if ref != title+" ("+url+")" {
		t.Fatalf("engine.BrowserPageRef() = %q", ref)
	}

	// open's sentence is parsed back for the agent-pages panel, so its shape is
	// a contract and not a preference.
	line := engine.BrowserOpenedLine(title, url)
	if !strings.Contains(line, ref) {
		t.Errorf("open does not use the shared ref: %q", line)
	}
	gotTitle, gotURL := engine.ParseBrowserOpened(line)
...
func TestAPageWithNoTitleIsNamedByItsAddress(t *testing.T) {
	if got := engine.BrowserPageRef("", "https://example.com/"); got != "https://example.com/" {
		t.Errorf("engine.BrowserPageRef(no title) = %q", got)
	}
	if got := engine.BrowserPageRef("", ""); got != "" {
		t.Errorf("engine.BrowserPageRef(nothing) = %q, want empty so callers can omit it", got)
	}

	_, url := engine.ParseBrowserOpened(engine.BrowserOpenedLine("", "https://example.com/"))
	...
	if _, url := engine.ParseBrowserOpened("เปิดแล้ว: Example (https://example.test"); url != "" {
