...
func (a *App) workbenchOpenBrowser(ctx context.Context, url string, newTab bool, owner string) (title, finalURL, duplicateOf string, err error) {
	...
	url, query := a.api.ResolveWorkbenchURL(url)
	if query != "" {
...
func (a *App) browserWhere(id AgentTabID) string {
	...
	return browserTabRef(id, engine.BrowserPageRef(t.meta()))
}
...
func (s *browserOpenSkill) open(ctx context.Context, url string, newTab bool) (skill.Output, error) {
	...
	out.Content = engine.BrowserOpenedLine(title, finalURL)
	if duplicateOf != "" {
	...
	// way a file put on the desk is (engine/workbench_desk.go handedOver): a
	// web page that is not a deck is shown rendered through THIS call, per the
	// desk tool's own guidance, so this is where the flag has to be set for
	// it. Whether the address is a file of this chat's own output folder is
	// the engine's to say — it is resolved against the project, the way the
	// open was.
	if rel := s.app.api.HandedOverFile(finalURL); rel != "" {
